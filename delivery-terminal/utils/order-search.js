...
export function parseQrPayload(raw) {
  ...
  if (/^MEAL-/i.test(text)) {
    return { type: 'qrCode', value: text.toUpperCase() };
  }
 ...
 * @param {string} qrCode
 */
async function resolveUserIdsByQrCode(qrCode) {
...
/**
 * Find ready orders by number, client name, or QR payload.
 ...
 */
export async function findDeliveryOrders(orders, { orderNumber = '', name = '', qrPayload = '' } = {}) {
  ...
  if (hasQr) {
    ...
    } else if (parsed.type === 'qrCode') {
      userIds = await resolveUserIdsByQrCode(parsed.value);
      if (!userIds.length) {
        return [];
      }
    } else {
  ...
  let matched = orders.filter(o => o.status === ORDER_STATUS.READY);
...
/**
 * @param {Array<{ id: string, userId: string, orderNumber: string, status?: string }>} orders
...
export function describeSearchResults(orders, usersById = {}) {
  return orders.map(o => ({
    ...
    clientName: usersById[o.userId]?.name
      || (o.userId === 'kiosk-guest' ? 'Гость киоска' : 'Клиент'),
    status: o.status,
  }));
}
